...
public class DbaseFileHeader {
    ...
    public void writeHeader(WritableByteChannel out) throws IOException {
      writeHeader(out, Charset.defaultCharset());
    }
     ...
     *                 If errors occur.
     */
    public void writeHeader(WritableByteChannel out, Charset charset) throws IOException {
        ...
        try {
            ...
            int tempOffset = 0;
            for (int i = 0; i < fields.length; i++) {
                byte[] fieldNameBuf = fields[i].fieldName.getBytes(charset);
                // write the field name
                for (int j = 0; j < 11; j++) {
                    if (fieldNameBuf.length > j) {
                      buffer.put(fieldNameBuf[j]);
                    } else {
                      buffer.put((byte) 0);
                    }
                    /*
